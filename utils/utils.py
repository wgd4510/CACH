...
def get_labels(data, suppress_console_info=True):
    """
    Get list of labels
...
def get_image_file_names(data, suppress_console_info=True):
    """
...
def get_captions(data, suppress_console_info=True):
    """
...
def calc_map_k(qB, rB, query_label, retrieval_label, k=None):
    ...
    :return:
    """
    num_query = query_label.shape[0]  # batchsize的大小
    map = 0.
    if k is None:
        k = retrieval_label.shape[0]
    for i in range(num_query):  # 遍历每一个查询词
        # 第i个元素的标签 和 数据库中所有标签进行矩阵乘，如果和第i个元素标签值一样，那么gnd当前元素值为1
        gnd = (query_label[i].unsqueeze(0).mm(retrieval_label.t()) > 0).type(torch.float).squeeze() 
        tsum = torch.sum(gnd)  # gnd大小为数据库大小4369，获取结果中和查询词类别一致的所有样本个数
        if tsum == 0:
            continue
        hamm = calc_hamming_dist(qB[i, :], rB)  # 计算第i个元素和数据库所有元素的汉明距离
        _, ind = torch.sort(hamm)  # 从小到大进行排序，并获取对应的序号，距离越小越相似
        ind.squeeze_()
        gnd = gnd[ind]   # 从类别是否一致的0-1矩形中按照 汉明距离大小进行排序
        total = min(k, int(tsum))  # 取前k个值
        # 生成 [1,2,3,4....18,19,20] 的矩形
        count = torch.arange(1, total + 1).type(torch.float).to(gnd.device)
        # torch.nonzero只返回不为0的序号，再取序号列表的前k个值。正常情况距离越小，类别一致gnd为1的概率越大
